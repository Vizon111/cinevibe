...
export default function Hero({ movies }: { movies: TmdbItem[] }) {
  ...
  const tCard = useTranslations("movieCard");
  ...
  const title = m.title || m.name || tCard("noTitle");
    ...
    <section className="relative h-[60vh] min-h-[420px] flex items-end overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1400px] w-full mx-auto px-4 lg:px-8 pb-12">
        <div className="max-w-xl flex flex-col gap-3">
          <p className="text-accent2 text-sm font-semibold tracking-wide uppercase">{t("titleOfTheDay")}</p>
          <h1 className="font-display text-5xl lg:text-6xl tracking-wide leading-[1.05] pt-1">{title}</h1>
          <p className="text-muted text-sm line-clamp-3">{m.overview || tCard("noOverview")}</p>
          ...
          <Link
            href={`/${locale}/title/${isTv ? "tv" : "movie"}/${m.id}`}
