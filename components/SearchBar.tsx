...
export default function SearchBar() {
  ...
  const [groupTitle, setGroupTitle] = useState("Популярно");
  ...
  const fetchSuggestions = useCallback(async (q: string) => {
    ...
    try {
      ...
      setGroupTitle(q ? "Результаты поиска" : "Популярно");
    } catch {
  ...
  function goToItem(item: TmdbItem) {
    ...
    setOpen(false);
    setQuery("");
    router.push(`/title/${isTv ? "tv" : "movie"}/${item.id}`);
  }
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-muted flex gap-1">
                        ...
                        <span>•</span>
                        <span>{year}</span>
                        <span className="opacity-60">{isTv ? "· сериал" : "· фильм"}</span>
                      </div>
                    </div>
                  </button>
                );
